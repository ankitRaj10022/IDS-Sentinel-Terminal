...
import os
for file in os.listdir("kddresults/dnn2layer/"):
  if file.endswith('.keras'):
    try:
        temp_model = load_model("kddresults/dnn2layer/"+file, compile=False)
        model.set_weights(temp_model.get_weights())
    except:
        model.load_weights("kddresults/dnn2layer/"+file, skip_mismatch=True, by_name=True)
...
best_model_path = "kddresults/dnn2layer/"+name[score.index(max(score))]
try:
...
np.savetxt("dnnres/dnn2predicted.txt", pred)
np.savetxt("dnnres/dnn2probability.txt", proba)
