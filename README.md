...
- Provides defensive network triage commands such as local ports, netstat, live intrusion scanning, DNS, process list, hash, file scan, IOC storage, and IOC hunting.
...
Network intrusion scan (this PC — live connections, IOCs, SSH auth logs):

```bash
# Scan ~8 seconds for suspicious listeners, bursts, IOC matches, and auth brute force.
ids-sentinel intrusions

# Longer sample and save JSON + text report under automation/product/intrusion_reports/
ids-sentinel intrusions --duration 20 --export

# JSON output for automation
ids-sentinel intrusions --json

# Static prevention guide for each intrusion type
ids-sentinel intrusions --guide

# Shell equivalents
intrusions
intrusions 15 export
intrusions guide
```
